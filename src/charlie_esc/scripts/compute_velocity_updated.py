...
# 1. Manejo dinámico de rutas (Arquitectura)
if len(sys.argv) < 2:
    print("Uso: python3 compute_velocity.py <ruta_a_carpeta_experimento>")
    sys.exit(1)

EXPERIMENT_DIR = Path(sys.argv[1])
ODOM_FILE = EXPERIMENT_DIR / "odom.csv"

if not ODOM_FILE.exists():
    print(f"[ERROR] No se encontró el archivo: {ODOM_FILE}")
    sys.exit(1)

# Cargar la odometría extraída del bag
...
# 2. Optimización Matemática: Forward speed in the vehicle body frame
# Proyección directa del vector de velocidad sobre el heading del vehículo
...
# Guardar en la carpeta del experimento
